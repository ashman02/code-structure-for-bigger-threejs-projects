/**
 * Why we created why.js file ? 
 * In this file you will find info about why we created a folder and what are the classes we created in it. 
 * 
 * ---------------------------
 * 
 * Why we created utils folder ? 
 * There are handy classed that are not necessarily related to WebGL or to Three.js. We are going to put them in /src/Experience/Utils folder.
 * 
 * --- Classes ---
 * 1. Sizes - This class will handle the sizes of the experience. It will include the width, the height and the pixel ratio. It will handle resizing as well. 
 * 2. EventEmitter - This class will handle events. It will include the on, off and trigger methods.
 * 
 */